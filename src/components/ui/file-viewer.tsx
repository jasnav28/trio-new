  ...
  return (
    ...
    >
      <AccordionPrimitive.Trigger
        className={cn(
          "flex items-center gap-1 rounded-md text-sm px-2 py-1 hover:bg-accent hover:text-accent-foreground cursor-pointer text-left w-full",
      ...
      >
        {expandedItems?.includes(value)
          ? openIcon ?? <FolderOpenIcon className="h-4 w-4 shrink-0 text-amber-500" />
          : closeIcon ?? <FolderIcon className="h-4 w-4 shrink-0 text-amber-500" />}
...
function File({
  ...
  return (
    <button
      disabled={!isSelectable}
      className={cn(
        "flex w-fit items-center gap-1 rounded-md px-2 py-1 text-sm transition-colors cursor-pointer text-left",
        isSelected && isSelectable && "bg-[#7342E2]/10 text-[#7342E2] dark:text-[#a882fa] font-semibold",
        !isSelectable
    ...
    >
      {fileIcon ?? <FileIcon className="h-4 w-4 text-sky-500 shrink-0" />}
...
function Tree({
  ...
  // Expand folders leading to the selected item automatically
  useEffect(() => {
    if (selectedId) {
      ...
      if (parts.length > 1) {
        ...
        setExpandedItems((prev) => {
          const unique = new Set([...(prev ?? []), ...foldersToExpand]);
...
function FileTree({
  ...
  return (
    <div className="w-full h-full border-r bg-card/40">
      <div className="p-3 border-b flex items-center gap-2 select-none">
        <FileCode className="h-4 w-4" />
        <span className="text-sm font-medium">{component.name} {component.version}</span>
      </div>
      <ScrollArea className="h-[380px] lg:h-[calc(100vh-320px)]">
        <div className="p-2">
          <Tree
            elements={tree}
            initialExpandedItems={[]} // Collapse by default
...
export default function ComponentFileViewer({
  ...
  const setActiveFile = setSelectedFile !== undefined ? setSelectedFile : setLocalSelectedFile;
  ...
  return (
    <ResizablePanelGroup
      ...
      className="min-h-[500px] lg:min-h-[600px] rounded-2xl border bg-card/25 overflow-hidden border-neutral-200 dark:border-neutral-850 shadow-xl"
    >
        ...
        {selected && (
          <div className="h-full flex flex-col">
            <FileHeader
              ...
              copied={copied}
            />
            <div className="flex-1 overflow-hidden bg-zinc-950/5 dark:bg-black/50">
              <ScrollArea className="w-full h-[380px] lg:h-[calc(100vh-320px)]">
                <ShikiViewer
