...
export function FocusRail({
  ...
  return (
    <div
      className={cn(
        "group relative flex h-[780px] w-full flex-col overflow-hidden transition-colors duration-300 outline-none select-none overflow-x-hidden",
    ...
    >
        ...
        <AnimatePresence mode="popLayout">
          ...
          >
            ...
            <div className={cn(
              ...
              isDark ? "from-neutral-950 via-neutral-950/50" : "from-neutral-50 via-neutral-50/50"
            )} />
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      {/* Main Stage */}
      <div className="relative z-10 flex flex-1 flex-col justify-between py-10 px-4 md:px-8 max-w-7xl mx-auto w-full">
        {header && (
        ...
        >
          {visibleIndices.map((offset) => {
            ...
            const brightness = isCenter ? 1 : (isDark ? 0.5 : 0.85);

            return (
              <motion.div
                key={absIndex}
                className={cn(
                  "absolute aspect-[3/4] w-[260px] md:w-[300px] rounded-2xl border-t shadow-2xl transition-shadow duration-300",
                  isDark ? "border-white/20 bg-neutral-900" : "border-black/5 bg-white",
                  isCenter ? (isDark ? "z-20 shadow-white/10" : "z-20 shadow-black/10") : "z-10"
                )}
                initial={false}
                animate={{
                  x: xOffset,
        ...
        <div className="mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row pointer-events-auto">
              ...
              >
                {activeItem.meta && (
                  <span className={cn(
                    ...
                    isDark ? "text-emerald-400" : "text-[#7342E2]"
                  )}>
                    {activeItem.meta}
                  </span>
                )}
          ...
          <div className="flex items-center gap-4">
            <div className={cn(
              "flex items-center gap-1 rounded-full p-1 ring-1 backdrop-blur-md",
              isDark ? "bg-neutral-900/80 ring-white/10 text-white" : "bg-white/80 ring-black/5 text-neutral-800"
            )}>
              <button
                onClick={handlePrev}
                className={cn(
                  "rounded-full p-3 transition active:scale-95 cursor-pointer",
                  isDark ? "text-neutral-400 hover:bg-white/10 hover:text-white" : "text-neutral-600 hover:bg-black/10 hover:text-black"
                )}
                aria-label="Previous"
              >
            ...
            {activeItem.href && (
              <a
                ...
                className={cn(
                  "group flex items-center gap-2 rounded-full px-5 py-3 text-sm font-semibold transition-all hover:scale-105 active:scale-95 cursor-pointer shadow-lg",
                  isDark ? "bg-white text-black hover:bg-neutral-100" : "bg-[#7342E2] text-white hover:bg-[#6232d1] shadow-purple-500/20"
                )}
              >
                Explore
