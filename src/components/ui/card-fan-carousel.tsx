      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            const cardContent = (
              <div className={`relative w-full h-full p-5 flex flex-col justify-between overflow-hidden text-left bg-white/95 dark:bg-[#0A0B10]/90 backdrop-blur-md rounded-[inherit] transition-all duration-300 ${card.isFeatured ? 'ring-[2.5px] ring-blue-500/60 shadow-[0_8px_30px_rgba(37,69,243,0.12)] dark:shadow-[0_0_35px_rgba(37,69,243,0.35)] border-blue-500/50' : 'border-slate-200 dark:border-slate-800/80'}`}>
                <div>
                  {/* Top Section: Title and Arrow Button */}
                  <div className="flex items-start justify-between gap-2 mb-4">
                    <h3 className="text-[16px] font-bold text-slate-850 dark:text-white font-sans leading-snug tracking-tight">
                      {card.title}
                    </h3>
                    <div className="w-7.5 h-7.5 rounded-full border border-slate-200 bg-slate-100/80 dark:border-slate-800 dark:bg-slate-950/60 flex items-center justify-center text-slate-400 hover:text-slate-900 hover:border-slate-350 dark:hover:text-white dark:hover:border-slate-700 transition-colors shrink-0 cursor-pointer">
                      <ArrowUpRight className="w-3.5 h-3.5" />
                    </div>
                  </div>

                  {/* Middle Section: Circular Icon & Description */}
                  <div className="flex items-start gap-3.5 mb-5">
                    <div className="w-10.5 h-10.5 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 text-white flex items-center justify-center shrink-0 shadow-[0_4px_12px_rgba(37,69,243,0.3)]">
                      {card.icon}
                    </div>
                    <p className="text-[10px] sm:text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed font-sans">
                      {card.desc}
                    </p>
                  </div>
                </div>
                
                {/* Bottom Section: Mockup area */}
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer">{cardContent}</a>
            ) : (
